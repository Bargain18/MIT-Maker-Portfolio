...
public class BackdropAuto extends LinearOpMode {
    ...
    public double TIMEOUT_RETRIEVE_PIXELS = 27; // give up on pixels after this 26
    ...
    public void runOpMode() {
        ...
        while (!isStopRequested() && !opModeIsActive()){
            ...
            if(gpad.shift.a) {distFlag = !distFlag;}
            ...
            if (gpad.start && gpad.dpad_right) yellowOffset = -1.5; 
            if (gpad.start && gpad.dpad_down) yellowOffset = 0.0;
            if (gpad.start && gpad.dpad_left) yellowOffset = 1.5;
            ...
            telemetry.addData("yellow offset", Math.abs(yellowOffset) + (yellowOffset < 0 ? " RIGHT" : " LEFT"));
            ...
            telemetry.addData("Distance Sensored Use", distFlag);
    ...
    public void run4() {
        if (yellowOffset > 0.01) yellowOffset -= 0.5;
        scorePurple(17, -46.5, 11.5, -0.4, 0.35);
        scoreYellow(-26.3, 42, 0.5);
    ...
    public void run5() {
        ...
        cycleFull(redCycleOut, 3, redCycleIn, 15);
        cycleFull(redCycleOut, 1, redCycleIn, 27); //changed to high score from mid
        cycleFull(redCycleOut3, 3, redCycleIn, 27, 0.07);
        park();
    }
    ...
    public void scoreYellow(double ty, double tx, double vel) {
        scoreYellow(ty, tx, vel, 25);
    }

    public void scoreYellow(double ty, double tx, double vel, int scLevel) {
        if (Math.abs(yellowOffset) > 0.01) {
            bot.setLevel(29);
        } else {
    ...
    public void cycleOut(Nav.Path path, int inLevel) {
        ...
        // }
        nav.setRampProfile(24, 15, 1, 0.3);
        while (opModeIsActive()) {
            nav.updateTracking();
            //nav.updateAprilTagPosition();
    ...
    public void cycleIn(Nav.Path path, int scLevel) {
        nav.setPath(path);
        nav.setRampProfile(32, 24, 1, 0.5);
        while (opModeIsActive()) {
            nav.updateTracking();
            nav.updateAprilTagPosition();
            double d = nav.getPathRemainingDist();
            //if (d < 1) break; // TODO: comment this out
    ...
    public void retrievePixels(Nav.Path opath, double danceOffset) {
        ...
        while(opModeIsActive()) {
            ...
            if(distFlag){
                ...
                if(nav.field.x < -72 + 7.75) {fwd = false;}
            }
            ...
            } else if (nav.moveToYXH(ty, wallx+14.5, 0.25 + danceOffset, 0.5) < 1) {
